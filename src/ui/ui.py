...
class Console:
    ...
    def fire_player(self, position):
        x, y = position[0], int(position[1]) - 1
        x = ord(x) - 65
        #the positoion will be "G7" for example, so position[0] is G
        #we use this because 65 is the ASCII code of 'A', and if we extract 65 from A it will give us 0, so the coordinate 0,
        #if we extarct 65 from B, it will give us coordinate 1 and so on.
